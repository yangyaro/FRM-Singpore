...
#wdir = "/Users/haerdle/Downloads/FRM_code_WKH"
#setwd(wdir)
...
#Preprocess the data
Mktcap_new=read.csv( "cap2.csv", header = TRUE )
Stock_Prices_new = read.csv("price2.csv", header = TRUE)
Macro_new = read.csv("SG2.csv", header = TRUE)
...
## 2. Save Results
#write.table(All_return, "Data_Returns_20200310.csv", row.names = F, col.names = T)
#write.table(Mktcap_Index, "Data_Mktcap_Index_20200310.csv", row.names = F, col.names = T)
#write.table(Mktcap_Value, "Data_Mktcap_Value_20200310.csv", row.names = F, col.names = T)
Time_End - Time_Start
...
#All_return=read.table( "Data_Returns_20200310.csv", header = TRUE)
#Mktcap_Index= read.table("Data_Mktcap_Index_20200310.csv",header = TRUE)
#Mktcap_Value=read.table("Data_Mktcap_Value_20200310.csv", header = TRUE)
...
## 8. Risk level
#dFRM_previous=read.csv("lambda_mean_AME.csv")
#colnames(dFRM_previous )=c('Date','FRM')
#dFRM_all=append(dFRM_previous[,2],FRM_series_final[,2])
